...
public int recursiveLinearSearch(int catNumToFind, int startIndex)
{
  if (startIndex < store.length) {
    return (store[startIndex].getCatNum() == catNumToFind) ? store[startIndex].getInventory() : recursiveLinearSearch(catNumToFind, startIndex+1);
  }
  return -1;
}
public int binarySearch(int catNumToFind)
{
  ...
  while (left <= right) {
    ...
    if (store[middle].getCatNum() == catNumToFind) return store[middle].getInventory();
    if (store[middle].getCatNum() > catNumToFind) right = middle-1;
    if (store[middle].getCatNum() < catNumToFind) left = middle+1;
  }
  return -1;
}
public int recursiveBinarySearch(int catNumToFind, int nLow, int nHigh)
{
  ...
  if (store[mid].getCatNum() < catNumToFind) return recursiveBinarySearch(catNumToFind, mid+1, nHigh);
  if (store[mid].getCatNum() > catNumToFind) return recursiveBinarySearch(catNumToFind, nLow, mid-1);
